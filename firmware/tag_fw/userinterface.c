...
void showCurrentTime(uint32_t t) {
    powerUp(INIT_EPD);
    selectLUT(EPD_LUT_DEFAULT);
    setColorMode(EPD_MODE_NORMAL, EPD_MODE_INVERT);
    clearScreen();
    ...
    epdPrintBegin(0, 270, EPD_DIRECTION_Y, EPD_SIZE_DOUBLE, EPD_COLOR_BLACK);
    epdpr(" %02d:%02d:%02d", h, m, s);
    epdPrintEnd();
    epdPrintBegin(48, 270, EPD_DIRECTION_Y, EPD_SIZE_DOUBLE, EPD_COLOR_RED);
    epdpr("%d'C %dmV", temperature, batteryVoltage);
    epdPrintEnd();
