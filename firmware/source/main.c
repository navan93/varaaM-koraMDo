...
void main(void)
{
	...
	while (1)
	{
		...
		pr("Sleeping now %lu   %02d:%02d:%02d  NextUpdate: %d\n", current_unix_time, h, m, s, next_minute);
		...
		else if (next_minute == m)// refresh the time every minute
		{
			next_minute = (m + 5) % 60;
			do_refresh = 1;
		}
		if (do_refresh)
		{
			...
			drawFullscreenMsg(timeString(current_unix_time), "Hello World");
		}
